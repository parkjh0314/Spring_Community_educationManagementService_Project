...
<script type="text/javascript">
	...
	$(document).ready(function() {
	    ...
	    //쓰기버튼
	    $("#marketBoardRegister").click(function(){
	    ...
	    */    
	     ...
	     // alert(contentval);
	 	   ...
	 	   var frm = document.marketBoardRegisterForm;
	 	   frm.price.value = $("input#price").val().replace(/,/g,'');
	 	   frm.action = '<%= ctxPath%>/marketAddEnd.sky';
	 	   frm.method = "POST";
	 	   frm.submit();
	    });
...
<div class="container"  align="left" class="form-group" id="register-div">
	<form class="form-inline" name="marketBoardRegisterForm" enctype="multipart/form-data">		
		<ul>
			<li><h2>${tableInfo.boardName}</h2></li>
			...
			<li>
				...
				<input type="hidden" name="fk_boardKindNo" id="fk_boardKindNo" value="${tableInfo.boardKindNo}" />
				<input type="hidden" name="fk_commuMemberNo" id="fk_commuMemberNo" value="${sessionScope.loginuser.commuMemberNo}" />
				<input type="hidden" name="fk_memberNo" id="fk_memberNo" value="${sessionScope.loginuser.fk_memberNo}" />
				<input type="hidden" name="writerIp" id="writerIp" value="${paraMap.writerIp}" />
				<input type="hidden" name="point" id="point" value="${loginuser.point}" />
				<input type="hidden" name="hi" id="hi" value="${paraMap.categoryNo}" />
			</li>
		</ul>
		<br>
		<div align="center">
			<button type="button" id="marketBoardRegister" >등록</button>
